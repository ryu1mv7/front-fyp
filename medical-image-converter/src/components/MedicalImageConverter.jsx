...
const MedicalImageConverter = () => {
  ...
  const [activeTab, setActiveTab] = useState('conversion'); // For main tabs
  const [infoTab, setInfoTab] = useState('metrics'); // For bottom info tabs
...
return (
  <div className="flex flex-col items-center p-6 bg-gray-900 text-white min-h-screen">
    ...
    <div className="w-full max-w-4xl bg-gray-800 rounded-lg p-6 shadow-lg">
      ...
      {/* Conversion Panel (shown by default) */}
        ...
        <>
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <div className="flex gap-2">
              ...
              >
                {isLoading ? <RefreshCw size={18} className="mr-2 animate-spin" /> : <ArrowRight size={18} className="mr-2" />} Convert
              </button>
            </div>
          </div>

          {error && (
            <div className="mb-4 p-3 bg-red-500 bg-opacity-20 border border-red-500 rounded text-red-300">{error}</div>
          )}

          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1 h-64 border-2 border-dashed border-gray-600 rounded flex items-center justify-center bg-gray-700" onDragOver={handleDragOver} onDrop={handleDrop}>
              {previewUrl ? (
                <img src={previewUrl} alt="Input" className="max-w-full max-h-full object-contain" />
              ) : (
